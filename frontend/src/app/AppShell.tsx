...
export interface ShellContext { species: Species | null; world: World | null; }
...
export function AppShell() {
  ...
  const links = [["/", "Visão geral"], [speciesPath, "Minha linhagem"], [`${speciesPath}#pressures`, "Pressões"], [`${speciesPath}#genome`, "Genoma"], ["/world", "Ecossistema"], ["/world", "Espécies"], ["/events", "Eventos"], ["/world/history", "História"], ["/legacy", "Legado"]] as const;
  return <div className="game-shell">
    ...
    <aside className="sidebar"><div className="nav-label">Centro evolutivo</div><nav>{links.map(([to, label], index) => <NavLink key={`${to}-${label}`} to={to} end={index === 0}><span>●</span>{label}</NavLink>)}</nav></aside>
    <main className="main-panel"><Outlet context={{ species, world: world.data } satisfies ShellContext}/></main>
